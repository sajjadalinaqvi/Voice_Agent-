...
- Silero VAD via torch.hub when available, fallback to webrtcvad.
- RealtimeSTT (AudioToTextRecorder) usage with guarded calls.
- Coqui TTS (FastPitch) for offline speech synthesis, fallback to placeholder tone.
- Flask web UI endpoints to start/stop and check status.
NOTE: This file avoids hard-coded secrets. Set GROQ_API_KEY via environment or the UI.
"""

# ... [All previous imports remain the same until TTS section] ...

# Replace TTS imports with simpler Coqui TTS implementation
import torchaudio
from TTS.api import TTS


class VITSTTS:
    """High-quality offline TTS using VITS model"""
    ...
    def __init__(self, sample_rate: int = 22050):
        ...
        try:
            # Initialize VITS model - lighter than XTTS but still high quality
            self.model = TTS(model_name="tts_models/en/ljspeech/vits",
                             progress_bar=False,
                             gpu=torch.cuda.is_available())
            print("[SUCCESS] VITS TTS initialized")
        except Exception as e:
    ...
    def synthesize(self, text: str) -> bytes:
        ...
        try:
            ...
            with BytesIO() as wav_buffer:
                self.model.tts_to_file(text=text, file_path=wav_buffer)
                wav_buffer.seek(0)
    ...
    def _fallback_beep(self, text: str) -> bytes:
        ...
        duration = min(len(text) * 0.05, 3.0)  # Max 3 seconds
...
class VoiceAgent:
    ...
    def __init__(self, config: VoiceAgentConfig):
        ...
        self.tts = VITSTTS(sample_rate=24000)  # Changed to CoquiTTS

        # ... [Rest of the VoiceAgent class remains exactly the same] ...
